...
docker compose exec web python manage.py migrate --fake cookbook 0001

echo "Applying fake cookbook migrations 0002..."
docker compose exec web python manage.py migrate --fake cookbook 0002

echo "Filling DB with basic categories..."
docker compose exec web python manage.py fill_categories
